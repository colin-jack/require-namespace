...
var create = function (associatedDir, name, moduleLoader) {
    try {
        ...
        moduleLoader = moduleLoader === undefined ? require('./DefaultModuleLoader') : moduleLoader;
...
var validateNamespaceArguments = function (directoryPath, name, moduleLoader, holderOfExisingNamespaces) {
    ...
    if (moduleLoader) {
        if (!moduleLoader.require || typeof moduleLoader.require != "function") {
            throw new ReferenceError("If provided the module loader must be an object with a method 'require'.");
        }
    }
